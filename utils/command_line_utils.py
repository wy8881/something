...
class CommandLineUtils:
    ...
    def parse_parameters(ca_file, cert, key, endpoint):
        cmdUtils = CommandLineUtils("PubSub - Send and receive messages through an MQTT connection.")
        cmdUtils.add_common_mqtt_commands()
        cmdUtils.add_common_topic_message_commands()
        cmdUtils.add_common_proxy_commands()
        ...
        cmdData = CommandLineUtils.CmdData()
        cmdData.input_port = int(8883)
        cmdData.input_endpoint = endpoint
        cmdData.input_cert = cert
        cmdData.input_key = key
        cmdData.input_ca = ca_file
        cmdData.input_clientId =  "test-" + str(uuid4())
        cmdData.input_message = cmdUtils.get_command(CommandLineUtils.m_cmd_message, "Hello World! ")
        cmdData.input_count = int(10)
        cmdData.input_is_ci = None
        return cmdData
